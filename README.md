...
AI-консультант для KeTai Consulting - специализированный бот для организации поставок и производства товаров из Китая "под ключ". Обслуживает каталог 40,000+ товаров с автоматическим управлением лидами.

**🎯 Специализация:** Китайские поставки и производство  
**📱 Каналы:** Telegram бот + веб-виджет (опционально)  
...
### 1. 🔍 Интеллектуальный поиск по каталогу
...
### 2. 🤖 Мультиязычное консультирование
...
### 3. 📈 Автоматическое управление лидами
- **Автоматическое создание** при наличии имени клиента И одном из условий:
  - Завершен диалог (таймаут 2 часа)
  - Позвали менеджера
  - Заказали товар/услугу
- **Уведомления менеджерам:** Автоматическая отправка в Telegram группу при создании лида
- **CRM:** Zoho с retry (макс 2 попытки, интервал 30 мин) - опциональное расширение
...
Система автоматически отправляет уведомления менеджерам в Telegram группу при создании лидов:

**Когда отправляются уведомления:**
- При ручном создании лида через форму "Связаться с менеджером"
- При автоматическом создании лида для неактивных пользователей (через 2 часа неактивности)
...
**Настройка:**
- `MANAGER_TELEGRAM_CHAT_ID` - ID группы для уведомлений менеджерам
- `ADMIN_TELEGRAM_IDS` - ID администраторов для критических уведомлений

### Критические уведомления
Система отправляет критические уведомления администраторам при:
- Ошибках подключения к базе данных
- Превышении лимитов AI расходов
...
## 👤 Поля лидов в Zoho CRM (опциональное расширение)
...
### Zoho CRM API v3 (возможное расширение)
- Create/Update Lead
...
# === УВЕДОМЛЕНИЯ ===
# Telegram chat ID для уведомлений менеджерам о новых лидах
MANAGER_TELEGRAM_CHAT_ID=-100ваш_chat_id

# Telegram ID администраторов для критических уведомлений (через запятую)
...
- **Система уведомлений:** `docs/notifications_system_guide.md`
- **Скрипты автоматизации:** `docs/scripts_documentation.md`
