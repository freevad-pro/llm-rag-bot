...
log "🧹 Очищаем старые Docker образы и кэш сборки..."
if [ "$REBUILD_NEEDED" = true ]; then
    ...
    # КРИТИЧНО: Очищаем кэш сборки (может занимать десятки GB!)
    log "🗑️ Очищаем кэш сборки Docker..."
    docker system prune -f > /dev/null 2>&1 || true
    
    # Показываем освобожденное место
    log "💾 Проверяем использование диска..."
